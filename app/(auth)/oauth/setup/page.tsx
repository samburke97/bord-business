...
export default function OAuthSetupPage() {
  ...
  useEffect(() => {
    ...
    initializeOAuthFlow();
  }, [session?.user?.id, session?.user?.status, status, router]);

  const handleSetupComplete = async () => {
    try {
      // Record OAuth profile completion in journey
      await fetch("/api/user/journey", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          step: OnboardingStep.OAUTH_PROFILE_SETUP,
          profileCompleted: true,
        }),
      });

      console.log("✅ OAuth profile completion recorded");
    } catch (error) {
      console.error("Failed to record journey step:", error);
    }
